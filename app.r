...
setkde.all$YEAR <- factor(setkde.all$YEAR, levels=c("2015","2016","2017","2018","2019","2020"))
               ...
               column(3,
                      wellPanel(
                        ...
                        checkboxInput(inputId = "checkbox", 
                                      ...
                                      value=TRUE),
                        checkboxInput(inputId = "checkbox.standardisation", 
...
server <- function(input, output){
  ...
  output$plot.Individual <- renderPlot({
    ...
    p2 <- p2 +  #what all plots have in common:
  ...
  output$plot.Ind.click_brush <- renderPrint({
      ...
      if(nrow(click)>0) {
        if(input$checkbox.standardisation==TRUE){
          ...
          click[,c(2:4,12,16,6:7,11)] #keep only interesting rows, and only rows without factors
        } else if (input$checkbox.standardisation==FALSE) {
          click <- click[order(click[,2] ), ] #order by START_DATE
  ...
  output$zoomplot <- renderLeaflet({
    ...
    if(dataInput()==1){ # display only relevant years
      ...
      colors <- c(rep("white",7),"red","white") #fill inside borders of circles
      labels <- c("-3 years", "-2 years", "-1 year", "settlement", "+1 year", "+2 years", "+3 years", "nest", "on migration")
      sizes <- c(rep(12,7),2,12)
      shapes <- "circle"
      margin.top <- c(rep(5,7),8,5) #margin from circles to each other, distance from top
      margin.left <- c(rep(0,7),2,0) #margin from circles to left side (to center the "nest" circle)
      borders <- c('#440154FF', '#39568CFF', '#1F968BFF', '#73D055FF', '#FDE725FF', 
                   'orange', 'chocolate', 'red', 'gray')
    } 
    ...
    #  ### make colour palette for Date and Year -----------------------------------------------
      ...
      ### legend for settlement.year, on.migration and nest location ### ----------------------
      colors <- c(rep("white",9),"red","white") #fill inside borders of circles
      labels <- c("-5 years", "-4 years", "-3 years", "-2 years", "-1 year", "settlement", "+1 year", "+2 years", "+3 years", "nest", "on migration")
      sizes <- c(rep(12,9),2,12)
      shapes <- "circle"
      margin.top <- c(rep(5,9),8,5) #margin from circles to each other, distance from top
      margin.left <- c(rep(0,9),2,0) #margin from circles to left side (to center the "nest" circle)
      borders <- c('#CC00CC', '#9900CC', '#440154FF', '#39568CFF', '#1F968BFF', '#73D055FF', '#FDE725FF', 
                   'orange', 'chocolate', 'red', 'gray')
    }
