...
#NA in some cases in gpsss.b2$TIME --> replace!
{
  ...
  gpsss.b2$TIME[54478] <- "2016-03-27 02:00:00" #SWIK55 2016-03-27 02:00:00
  gpsss.b2$TIME[58151] <- "2017-03-26 03:00:00" #SWIK55 2017-03-26 03:00:00
}
...
write.table(set_rel_years, paste0(pathKML, "/KDE_AllSettlers_per_5days_2ppd_m_allbirds_relYears.csv"), row.names=F, sep=";")
...
pathKML <- paste0("/Users/Lara/Documents/_Jobs/2020 Sempach/Settlement Protocol/QGIS settlement protocol/Kernel Density Estimation")
settlers <- read.table("/Users/Lara/Documents/_Jobs/2020 Sempach/Settlement Protocol/QGIS settlement protocol/Settlers_list_Lara_all.csv", header=T, dec=".", sep=";")
setkde <- read.table(paste0(pathKML, "/KDE_AllSettlers_per_5days_2ppd_m_allbirds_relYears.csv"), header=T, dec=".", sep=";") #only relevant years
setkde.all <- read.table(paste0(pathKML , "/KDE_AllSettlers_per_5days_2ppd_m_allbirds.csv"), header=T, dec=".", sep=";")
...
shiny::runGitHub('RKsettlement2021_ggmap', 'LaGrande27', ref="main")
shiny::runGitHub('RKsettlement2021_leaflet', 'LaGrande27', ref="old_2graphsOnly")
shiny::runGitHub('RKsettlement2021_leaflet', 'LaGrande27', ref="master") #latest
...
sepo0 <- read.table(paste0(pathKML, "/Settlement_Potatoes_Dates.csv"), header=T, dec=".", sep=";") #which dates to take
  ...
  for (i in seq_along(sepo_t$Type)) {
    ...
    if (!is.na(sepo_t$Start[i])) {
      if (sepo_t$cutoff[i]==6 | sepo_t$cutoff[i]==20) {
        ...
        if(sepo_t$TransmGSM[1]!="KIWS14" & sepo_t$TransmGSM[1]!="KIWS53") {
        ...
        } else if (sepo_t$TransmGSM[1]=="KIWS53") { #one point in June for that kite is not at all at the settlement place
          donttake <- as.Date("2020-06-11", origin="1970-01-01")
          `%nin%` = Negate(`%in%`)
          results$LON[i] <- currentKiteGPS %>% filter(as.Date(DATE) %nin% donttake) %>% filter(as.Date(DATE) %in% filt.Date.6km2) %>% summarize(lon = mean(LON)) %>% round(digits=6)
          results$LAT[i] <- currentKiteGPS %>% filter(as.Date(DATE) %nin% donttake) %>% filter(as.Date(DATE) %in% filt.Date.6km2) %>% summarize(lat = mean(LAT)) %>% round(digits=6)
        }
...
View(result)
write.table(result, paste0(pathKML, "/Settlement_Potatoes_Locations.csv"), row.names=F, sep=";")
...
combine <- settl[which(settl$TransmGSM=="BC38EB1D" | settl$TransmGSM=="SWIK88"),] # merging the centroid for one tagged couple (Brünisried_Wilersguet)
combine[nrow(combine) + 1,] = list(paste(combine$bird_ID[1],combine$bird_ID[2], sep=""),
                                   ...
                                   mean(combine$LON),
                                   mean(combine$LAT))
settl <- rbind(settl, combine[3,]) %>% filter(TransmGSM!="BC38EB1D" & TransmGSM!="SWIK88")

settl$centroid.ID <- paste0(settl$bird_ID, sep="_", substr(settl$Type,1,1))


write.table(settl, paste0(pathKML, "/Settlement_Locations.csv"), row.names=F, sep=";")
